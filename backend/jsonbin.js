...
// Render uses Node 18+ by default (has global fetch).
// This helper falls back to a dynamic import if fetch is missing.
...
async function jsonbinGetLatest() {
  ...
  const res = await fetchFn(`${BASE_URL(BIN_ID)}/latest`, {
    headers: {
...
async function jsonbinPut(record) {
  ...
  const res = await fetchFn(`${BASE_URL(BIN_ID)}`, {
    method: 'PUT',
    headers: {
